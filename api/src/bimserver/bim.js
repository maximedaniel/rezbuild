...
export default class Bim {
	...
	getAllAuthorizedProjectsOfUser(uoid){
	  return new Promise((authorizedProjects, err) =>
	    this.client.call( "ServiceInterface", "getAllAuthorizedProjectsOfUser",
	    {uoid : uoid}, authorizedProjects, err));
	}
...
export default class Bim {
    ...
    init() {
    console.log(this.name, this.client)
    this.api.init((_client, version) => {
      this.client  = _client
      this.client.login( 'm.daniel@estia.fr', 'Avatar56.', this.onLogin)
    });
    }
    onLogin( data ) {
       console.log(this.name, this.client)
      this.createProject('MaximeDANIEL')
    }
	createProject(name){
	  this.client.call( "ServiceInterface"
	  , "addProject"
	  , { projectName : name, schema : 'ifc2x3tc1' }
	  , function(project){console.log("project "+ name + " created")}
	  )
	}
}

function onLogin( data ) {
  client.call( "ServiceInterface"
             , "addProject"
             , { projectName : 'TestBIMProject', schema : 'ifc2x3tc1' }
             , onGetProject
             , warn
  )
...
function onGetProject( data ) {
  log( "onGetProject ", data)
  client.call( "ServiceInterface"
             , "getAllProjects"
             , { onlyTopLevel : true, onlyActive : false }
  ...
  if( data.length > 0 ) {
    ...
    projectInfo.poid = projectInfo.data.oid
    projectInfo.roid = projectInfo.data.lastRevisionId
    getRezbuildPropertySet( )
  }
 }
//Get or create the propertySet
function getRezbuildPropertySet( ) {
  client.call( "LowLevelInterface"
             , "getDataObjectsByType"
             , { roid : projectInfo.roid
...
function onGetPropertySets( data ) {
  for( var propertySet of data ) {
    if( propertySet.name.indexOf("Rezbuild") >= 0 ) {
      console.log( propertySet )
      projectInfo.RezbuildPropertySet = propertySet
    }
...
function createPropertySet( tid, data, next ) {
  client.call( "LowLevelInterface"
              , "createObject"
              , { "tid": tid
                , "className": "IfcPropertySet"
...
function setPropertySetName( tid, oid, next ) {
  client.call( "LowLevelInterface"
              , "setStringAttribute"
              , { "tid": tid
                , "oid": oid
                , "attributeName": "Name"
                , "value": "RezbuildProperties"}
              , ( data ) => {
                  next( oid )
                }
              , warn
...
function perfomTransaction( callbacks, done ) {
  console.log( "perfomTransaction")
  if( callbacks instanceof Function ) {
    callbacks = [ callbacks ]
  }
  if( callbacks instanceof Array ) {
    let processor = ( tid, callbacks ) =>  {
      console.log( "tid", tid )
      let next = ( data )=>{
        console.log( "Next", callbacks.length  )
        if( callbacks.length > 0 ) callbacks.shift()( tid, data, next )
        else
          client.call(  "LowLevelInterface"
                   , "commitTransaction"
                   , { tid : tid
